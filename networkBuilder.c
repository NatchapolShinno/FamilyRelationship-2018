    ...
    if (pFp == NULL)
       ...
       if (fgets(inputline,sizeof(inputline),pFp) != NULL)
           ...
           if (vertexCount == 0)  /* bad format */
               ...
               { 
               if (bVerbose)
                   fprintf(stdout,"Successfully initialized network... ");
...
/** I'm edited this function in 'sscanf VERTEX %s %s'. I declace new variable call "gender"
to store the variable gender of each person. And I change some printf function.
*/
...
void processCommand(char* command, int bVerbose)
{
    ...
    if (strncmp(command,"VERTEX",strlen("VERTEX")) == 0)
       {
       //EDITED
       sscanf(command,"VERTEX %s %s",keystring1,keystring2);
       ...
       else
	 {
         ...
         newString = strdup(keystring2);  /* use the key as data as well */
